import {} from 'react'
import {Button, Heading, Wonder} from '../components'
...
function CardSection2({
  ...
  const wraperClass = classNames('flex flex-col items-center',
  'text-center',
  'min-h-60',
  `bg-no-repeat bg-center bg-55% bg-top`)

  const titleClass= classNames(
    'text-2xl text-primary-500 font-black mb-3',titleClassNames)
  const textClass = classNames("max-w-96 text-sm",textClassNames)

  return <div className={wraperClass} style={{backgroundImage:`url(${blobSrc})`}}>
...
export default function Services() {
  ...
  return <>
    ...
    <section className='bg-tertiary-500 py-4 px-24'>
      <Heading heading="YOUR BRAND" subHeading="our focus" className="text-center my-4"/>
      <div className="flex justify-between">
        <CardSection2 {...section3CardList[0]} textClassNames="max-w-80" titleClassNames="max-w-52"/>
        <CardSection2 {...section3CardList[1]} textClassNames="max-w-80" titleClassNames="max-w-52"/>
        <CardSection2 {...section3CardList[2]} textClassNames="max-w-80" titleClassNames="max-w-52"/>
      </div>

      <div className="rounded-[4rem] my-16 p-20 pb-16 bg-secondary-500">
        <Heading 
          heading="WE MAKE YOUR SALES GO"  
          subHeading="through the roof" subHeadingClassName="text-tertiary-500"/>
        <Button className="bg-primary-500 mt-14" arrow>Learn More</Button>
      </div>

      <div className="flex items-center justify-between">
        <Heading heading="Platform" subHeading="to help you" />
        <div className='ml-16 flex flex-wrap gap-6 justify-end'>
          {platforms.map((el)=>(
    ...
    <img src="./assets/wave12.svg" alt="wave" className="bg-tertiary-500 text-white"/>
    
    <section>Numbers don't lie</section>
    <section>EXPLORE OUR bugs</section>
    <Wonder />
  </>
}
