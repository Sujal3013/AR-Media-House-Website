export default function FounderCard({
  ...
  return (
    <div className="md:basis-[46%] lg:basis-[40%] shadow-lg hover:cursor-pointer rounded-2xl overflow-hidden ceo-container">
      <img
        src={imageSrc}
        alt={founderName}
        className="ceo-image md:max-h-[26rem]"
      />

      <div
        className={`w-fit min-w-[50%] pl-2 md:pl-3 pr-4 md:pr-6 py-1 mt-1.5 rounded-r-full ${
          index & 1
            ? "bg-secondary-500 text-tertiary-500"
            : "bg-primary-500 text-secondary-500"
        } whitespace-nowrap text-lg sm:text-2xl font-semibold relative z-10`}
      >
        {founderName}
      </div>
      ...
      <div className="p-4 pt-0 text-justify">
        <p className="my-3 text-xs sm:text-sm">{description}</p>
      </div>
    </div>
  );
}
