...
export default function ExploreBlogList({
  ...
  const classes = classNames(
    "min-w-[11.5rem] max-w-72 sm:max-h-[475px] h-[430px] sm:h-[445px] lg:h-[460px] text-secondary-500 flex flex-col items-start justify-evenly relative overflow-hidden gap-2 hover:cursor-pointer",
    {
      [className]: className,
    }
  );
  ...
  return (
    ...
    >
      ...
      />
      {blog.text && (
        <h1 className="font-medium sm:font-semibold text-base sm:text-lg md:text-xl">
          {blog.heading}
        </h1>
      )}
      {blog.text && <p className="text-xs">{blog.text}</p>}
      {
        <div className="flex flex-col h-12 overflow-hidden">
          <motion.div
            initial={{ y: 0, opacity }}
            animate={hover ? { y: "-100%", opacity } : {}}
          >
            
            <Button className="!pl-px" arrow to={`/case-studies/${blog.id}`}>
              learn more
            </Button>
